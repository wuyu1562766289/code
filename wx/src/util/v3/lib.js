...
IMService.prototype.onOpen = function () {
    ...
    this.socket.onclose = function() {
        if (this.observer != null && "handleDisconnect" in this.observer){
            this.observer.handleDisconnect();
        }
        self.onClose();
    };
...
IMService.prototype.handleMessage = function(msg) {
    ...
    } else if (cmd == IMService.MSG_GROUP_RT) {
        if (this.observer != null && "handleGroupRTMessage" in this.observer) {
            this.observer.handleGroupRTMessage(msg);
        }
...
IMService.prototype.onMessage = function (data) {
    ...
    if (cmd == IMService.MSG_IM) {
        ...
        msg.timestamp = ntoh64(buf, pos);
        pos += 8;

        //msgid
        pos += 8;

        msg.content = utf8.decodeUTF8(buf.subarray(IMService.HEADSIZE + 32, IMService.HEADSIZE + len));
    } else if (cmd == IMService.MSG_GROUP_IM) {
        msg.isSelf = isSelf;
...
IMService.prototype.sendFail = function () {
    for (var seq in this.messages) {
        var msg = this.messages[seq];
        if (this.observer != null && "handleMessageFailure" in this.observer){
            this.observer.handleMessageFailure(msg);
...
IMService.prototype.sendGroupRTMessage = function (msg) {
    if (this.connectState != IMService.STATE_CONNECTED) {
        return false;
    }
    var content = utf8.encodeUTF8(msg.content);
    ...
    var r = this.send(IMService.MSG_GROUP_RT, buf);
    if (!r) {
        return false;
    }
    return true;
